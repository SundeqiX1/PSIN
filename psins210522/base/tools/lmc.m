function lmc(line1, line2, unit, iscut)
% Line Move & Compare:
%   <-, -> :  for x-axis move
%   ^, v   :  for y-axis move
%   -, =   :  for change parameter 'dxy' by /*10
%
% Prototype: lmc(line1, line2)
...
global kpress_para
    ...
    if iscut
        t1 = min(line1(1,end),line2(1,end));  t2 = min(line1(end,end),line2(end,end));
    ...
    myfig;
    subplot(211), plot(kpress_para.xy1(:,1), kpress_para.xy1(:,2), kpress_para.xy2(:,1), kpress_para.xy2(:,2)); grid on;
    err = lineerr(kpress_para.xy1, kpress_para.xy2);
    subplot(212), plot(err(:,1), err(:,2), 'r'); grid on;
    hf = gcf;
